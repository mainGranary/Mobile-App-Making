...
function HomeScreen({navigation}){
    ...
    //처음 화면이 그려질 때 상태에 데이터가 준비되엇는지 관리
    ...
    //선택한 카테고리에 따라 cateState에 데이터 재할당
    // const categorization=(cate)=>{
    //     if(cate=="전체보기") setCateState(state);
    // }
    ...
    const renderRightActions=(dragX,scrollRef)=>{
                    ...
                    function({item, index}){
                        ...
                        return( 
                            ...
                            <GestureHandlerRootView>
                                <Swipeable renderRightActions={dragX=>renderRightActions(dragX,scrollRef)}>
                                    ...
                                    <TouchableOpacity onPress={function(){navigation.navigate("GifticonInfo",{key: gifticonList[index], index:index}), console.log(index)}}>
                                            ...
                                            <View style={{borderColor:'gray', borderWidth:2, borderRadius:10, flexDirection:'row', margin:10, padding:10, paddingHorizontal:20, alignItems:'center', justifyContent:'space-between'}}>
                                                ...
                                                <Text style={{marginLeft:20, margin:2, fontSize:25, fontWeight:'bold', color:color}}>D-{item.result}</Text>
                                            </View>
