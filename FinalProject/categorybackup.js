...
import {Text, View, ScrollView, TextInput, FlatList, StyleSheet, Button, TouchableOpacity, Image, Platform,Animated } from 'react-native';
...
function HomeScreen({navigation}){
    ...
    useEffect(function(){
        load_data();
        setTimeout(()=>{
            setState(gifticonList);
            setCateState(gifticonList);
            console.log("실행완료")
    ...
    return(
        <View style={{flex:1, backgroundColor:'white'}}>
            ...
            <ScrollView style={{margin:20}} horizontal indicatorStyle={"white"}>
            <TouchableOpacity style={{margin:20}} onPress={()=>{categorization('전체보기')}}><Text>전체보기</Text></TouchableOpacity>
            <TouchableOpacity style={{margin:20}} onPress={()=>{categorization('베이커리/카페')}}><Text>베이커리/카페</Text></TouchableOpacity>
            <TouchableOpacity style={{marign:20}} onPress={()=>{categorization('패스트푸드')}}><Text>패스트푸드</Text></TouchableOpacity>
            <TouchableOpacity style={{margin:20}} onPress={()=>{categorization('편의점')}}><Text>편의점</Text></TouchableOpacity>
            <TouchableOpacity style={{margin:20}} onPress={()=>{categorization('잡화')}}><Text>잡화</Text></TouchableOpacity>
          </ScrollView>
                    ...
                    function({item, index}){
                        ...
                        return( 
                            ...
                            <GestureHandlerRootView>
                                <Swipeable renderRightActions={dragX=>renderRightActions(dragX,index)}>

                                    <TouchableOpacity onPress={function(){navigation.navigate("GifticonInfo",{key: gifticonList[index], index:index}), console.log(index)}}>
