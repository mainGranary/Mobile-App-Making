...
function AddScreen({navigation}){
    ...
    return(
        <ScrollView>
            ...
            <Button title="haha" onPress={function(){navigation.navigate("List", {gifticonList:gifticonList})}}/>
...
function ListScreen({route, navigation}){
    ...
    for(var i=0; i<array.length; i++){
        var dday=new Date(Number(array[i].date.year), Number(array[i].date.month)-1, Number(array[i].date.day));
        var gap=dday.getTime()-today.getTime();
        daycount=Math.ceil(gap/(1000*60*60*24));
        array[i].result=daycount;
        console.log(array);

        if(array[i].result<=7){
            print[i]=<Text style={{color:'red'}}>D-{array[i].result}</Text>
        }
        else{
            print[i]=<Text>D-{array[i].result}</Text>
        }
        
        var a=<View style={{flexDirection:'row', margin:10}}>
            <Image source={{uri:array[i].image}} style={{width:50, height:50, marginLeft:20, marginTop:10}}/>
            <Text>{array[i].category}</Text>
            <Text>{array[i].title}</Text>
            <Text>{array[i].date.year}</Text>
            <Text>{array[i].date.month}</Text>
            <Text>{array[i].date.day}</Text>
            <Text>{print[i]}</Text>
        </View>
        L.push(a);
    }
